...
const Hero = () => {
    ...
    const { router, getToken, axios, setSearchedCities, user } = context;
    ...
    const onSearch = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (user) {
            await axios.post("/api/user/storeSearchedCities", { recentSearchedCities: destination }, {
    ...
    return (
        <div className={`flex flex-col items-start justify-center px-6 md:px-16 lg:px-24 xl:px-32 text-white bg-[url("/hotel_image.jpeg")] bg-no-repeat bg-cover opacity-90 bg-center h-screen`}>
            ...
            <form onSubmit={onSearch} className='mt-8 bg-white text-gray-500 rounded-lg px-6 py-4  flex flex-col md:flex-row max-md:items-start gap-4 max-md:mx-auto'>
                <div>
                    <div className='flex items-center gap-2'>
                        <Image src={assets.calenderIcon} alt="" className="h-4" />
